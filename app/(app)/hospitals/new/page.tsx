...
export default function NewHospitalPage() {
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(async () => {
      setSearching(true);
      const res = await fetch(`/api/place-listings?type=hospital&q=${encodeURIComponent(query)}`).catch(() => null);
  ...
  const mapsUrl = name
    ? `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(name + " 動物病院")}`
    : null;
            ...
            <button key={l.id} onClick={() => selectListing(l)} className="text-left w-full">
              <Card className="flex items-center gap-3 active:opacity-70 transition-opacity">
                {l.photoUrl ? (
                  ...
                  <div className="w-12 h-12 rounded-xl bg-app-bg flex items-center justify-center text-2xl flex-shrink-0">🏥</div>
                )}
